...
Changed apiPort and port, respectively, in `route.js` and `api-server.js` in order to host API on 4001 (avoiding conflict with React SPA).
- Had to add a callback uri for `http://localhost:4040/api/auth/callback` else a callback error occured when redirecting to the login screen (this is differnet than the React SPA where we only need `http://localhost:{port}`) 
